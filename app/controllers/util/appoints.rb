...
class Appoints
      ...
      if ((account.account_balance - tax_value - amount_debt) >= 0)
        ...
        AccountTransaction.transaction do
          ...
          @lancto_origem = AccountTransaction.create(account_id: origin_account_id,
                                          date: Date.current, description: 'Debitado para transferência', tr_type: 'D', amount: amount_debt , doc_ref: "Conta: #{dest_account_id}" )
          # Tax Debt
          @lancto_origem = AccountTransaction.create(account_id: origin_account_id,
                                                     date: Date.current, description: 'Taxa referente à transferência', tr_type: 'D', amount: tax_value )
          # Crediting the amount in the destiny Account
          @lancto_destino = AccountTransaction.create(account_id: dest_account_id,
                                           date: Date.current, description: 'Transferência', tr_type: 'C', amount: amount_debt, doc_ref: "Transf. da Conta: #{origin_account_id}"  )
  ...
  #Apllies the taxes on transferences
